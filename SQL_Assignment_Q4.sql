...
SELECT 
    ...
    MAX(signup_date) AS latest_update,
    MIN(signup_date) AS older_update,
    DATEDIFF(MAX(signup_date), MIN(signup_date)) AS diff_in_days  -- to find the diff days between the dates
FROM email_signup
